...
11. **Optional world-context scene-setter (ck3_chronicler-8ek slice 1).**
    The user prompt may contain a "World context" block describing the
    character's de jure region — their kingdom, peer kingdoms, and
    cross-currents (foreign-held holdings) at the time of their death.
    When that block is present, OPEN the biography with one short
    scene-setter paragraph (3–5 sentences) framing the character's
    neighbourhood from those facts. Use the kingdom and ruler names
    exactly as supplied; the same naming rails as constraint 1 apply.
    Do NOT invent additional realms, rulers, holdings, wars, or
    cross-currents — if a fact isn't in the World context block, it
    isn't yours to write. After the scene-setter paragraph, continue
    with the biography proper as instructed in 1–10. When the World
    context block is absent (a character whose realm we couldn't
    resolve), open as you would a v2-style biography directly.
